<script>
    export default {
        ...
        methods: {
            // 编辑模板
            getEditTemplate (h) {
                return !this.$scopedSlots.input ? '' : h('div', {
            ...
            // 文本模板
            getTemplate (h) {
                return !this.$scopedSlots.default ? '' : h('div', {
        ...
        render (h) {
            let template = ''
            if (this.useTooltip) {
                template = this.editable ? this.getEditTemplate(h) : this.getTemplate(h)
                ...
                const slot = this.editable ? 'input' : 'default'
                template = this.$scopedSlots[slot] ? this.$scopedSlots[slot]() : ''
            }
